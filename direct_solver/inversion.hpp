...
class AMD_Inversion : public IExternalSolver <TAlgebra>,
    public AMD_Solver {
public:
    using matrix_t = CPUAlgebra::matrix_type;
    using vector_t = CPUAlgebra::vector_type;

    AMD_Inversion() = default;

    ~AMD_Inversion() override {
        clear();
    };
    ...
    [[nodiscard]] const char * double_name() const override {  return "AMD_Inversion"; };
...
template <typename TAlgebra>
void AMD_Inversion<TAlgebra>::double_init(const matrix_t &mat){
    solver_.Clear();
...
template <typename TAlgebra>
bool AMD_Inversion<TAlgebra>::double_apply(vector_t &c, const vector_t &d){
